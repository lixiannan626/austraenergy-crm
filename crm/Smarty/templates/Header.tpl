...
<head>
	...
	{/if}
	{if $MODULE_NAME neq 'InstallArrangement' &&  $MODULE_NAME neq 'InstallPaperwork'  &&  $MODULE_NAME neq 'StockManager' && $MODULE_NAME neq 'Standardization' && $MODULE_NAME neq 'QuotePrice' && $MODULE_NAME neq 'SimpleSettings'}
	...
	{* vtlib customization: Inclusion of custom javascript and css as registered *}
	{if $HEADERSCRIPTS && $MODULE_NAME neq 'QuotePrice'}
	...
	<tr>
		<td valign=top><img src="{'crm-header.png'|@vtiger_imageurl:$THEME}" alt="vtiger CRM" title="vtiger CRM" border=0 height="53"></td>
	...
	<td class=small nowrap> 
		...
		<tr>
			...
			{foreach key=maintabs item=detail from=$HEADERS}
			 {if $maintabs ne "Marketing" && $maintabs ne "My Home Page"}
		...
		<table border=0 cellspacing=0 cellpadding=0 id="search" style="border:1px solid #999999;background-color:white">
		   <tr>
			...
			<td style="height:19px;background-color:#ffffef" nowrap>
				<a href='javascript:void(0);' onclick="UnifiedSearch_SelectModuleForm(this);"><img src="{'settings_top.png'|@vtiger_imageurl:$THEME}" align='left' border=0></a>
