	...
	<td width=100% nowrap>
					...
					{if $CHECK.EditView eq 'yes'}
			        		{if $MODULE eq 'Calendar'}
                	   			 ...
                	   			 {else}
			                       	...
			                       	{/if}
					{else}
						<td style="padding-right:0px;padding-left:10px;"><img src="{$IMAGE_PATH}btnL3Add-Faded.png" border=0></td>	
					{/if}
									
						<td style="padding-right:10px"><img src="{$IMAGE_PATH}btnL3Search-Faded.png" border=0></td>
			...
			<!-- Calendar Clock Calculator and Chat -->
				...
				<tr>
					{if $CALENDAR_DISPLAY eq 'true'} 
						...
						{else}
                                                {*<td style="padding-right:0px;padding-left:10px;"><img src="{$IMAGE_PATH}btnL3Calendar-Faded.png"></td>*}
                                                {/if}
					{else}
						{if $CHECK.Calendar eq 'yes'} 
					...
					{if $WORLD_CLOCK_DISPLAY eq 'true'} 
 		                                        ...
 		                                        {if $CHAT_DISPLAY eq 'true'} 
 		                                            {*    <td style="padding-right:0px"><a href="javascript:;" onClick='return window.open("index.php?module=Home&action=vtchat","Chat","width=600,height=450,resizable=1,scrollbars=1");'><img src="{$IMAGE_PATH}tbarChat.png" alt="{$APP.LBL_CHAT_ALT}" title="{$APP.LBL_CHAT_TITLE}" border=0></a> *}
 		                                        {/if} 
													{*</td>*}
		...
		<td class="small">
			...
			{if $MODULE eq 'Vendors' || $MODULE eq 'HelpDesk' || $MODULE eq 'Contacts' || $MODULE eq 'Leads' || $MODULE eq 'Accounts' || $MODULE eq 'Potentials' || $MODULE eq 'Products' || $MODULE eq 'Documents' || $CUSTOM_MODULE eq 'true' }
				...
				{else}	
					<td style="padding-right:0px;padding-left:10px;"><img src="{$IMAGE_PATH}tbarImport-Faded.png" border="0"></td>	
				{/if}	
				...
				{else}	
					<td style="padding-right:10px"><img src="{$IMAGE_PATH}tbarExport-Faded.png" border="0"></td>
                {/if}
			{else}
				<td style="padding-right:0px;padding-left:10px;"><img src="{$IMAGE_PATH}tbarImport-Faded.png" border="0"></td>
                <td style="padding-right:10px"><img src="{$IMAGE_PATH}tbarExport-Faded.png" border="0"></td>
			...
			{if $MODULE eq 'Contacts' || $MODULE eq 'Leads' || $MODULE eq 'Accounts'|| $MODULE eq 'Products'|| $MODULE eq 'HelpDesk'|| $MODULE eq 'Potentials'|| $MODULE eq 'Vendors'} 
				{if $VIEW eq true}
					<td style="padding-right:10px"><a href="index.php?module={$MODULE}&action=FindDuplicateRecords&button_view&list_view"><img src="{$IMAGE_PATH}findduplicates.png" alt="{$APP.LBL_FIND_DUPICATES}" title="{$APP.LBL_FIND_DUPLICATES}" border="0"></a></td>
				{else}
					<td style="padding-right:10px"><img src="{$IMAGE_PATH}FindDuplicates-Faded.png" border="0"></td>	
				{/if}
			{else}
				<td style="padding-right:10px"><img src="{$IMAGE_PATH}FindDuplicates-Faded.png" border="0"></td>
			{/if}
