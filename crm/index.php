...
if((!$viewAttachment) && (!$viewAttachment && $action != 'home_rss') && $action != 'Tickerdetail' && $action != $Ajx_module."Ajax" && $action != "chat" && $action != "HeadLines" && $action != 'massdelete'  &&  $action != "DashboardAjax" && $action != "ActivityAjax")
{
	...
	
	{
		...
		echo "<table border=0 cellspacing=0 cellpadding=5 width=100% class=settingsSelectedUI2 style='border-top: 1px solid #f2f2f2;' >";
		echo "<tr><td class=small align=left><span style='color: rgb(153, 153, 153);'>&copy; ".date('Y')." Go For Solar CRM</span></td>";
		echo "<td class=small align=right><span style='color: rgb(153, 153, 153);'></span> $statimage</td></tr></table>";
