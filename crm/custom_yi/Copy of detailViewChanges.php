...
//else if($current_user->id != 10)
{
    ...
    for (var i in elems)
    {
        if((" "+elems[i].className+" ").indexOf("dvtCellInfo") > -1)
        {
			...
			var count = 0;
			//Determine the fields to be editable by Sales
			if(idd == "mouseArea_Deposit Date" || idd == "mouseArea_Deposit Amount" || idd == "mouseArea_Invoice Number" || idd == "mouseArea_Full Payment Option" || idd == "mouseArea_Total Amount" || idd == "mouseArea_Full Payment Date" || idd == "mouseArea_Full Payment Confirmed" || idd == "mouseArea_STCs Claimed by")
			{	
				//alert(idd);
				//count = count+1;
				elems[i].setAttribute("onmouseover","");
