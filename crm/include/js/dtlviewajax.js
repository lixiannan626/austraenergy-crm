...
function dtlViewAjaxSave(fieldLabel,module,uitype,tableName,fieldName,crmId)
{
	...
	/*
		Following codes are to:
		1. Reject submission when trying to update invoice but not sale
		2. Reject submission when trying to assign team when invoice is empty
		
	*/
	if(fieldLabel == "Invoice Number")
	{
		var sp = "||"+document.getElementById("dtlview_Sales Progress").innerHTML;
		if(sp.indexOf("Sale") <= 0)
		{
			alert("Please change sales progress to 'Sale' first in order to add invoice number.");
			return false;
		}
	}
	if(fieldLabel == "Installation Team")
	{
		var inno = document.getElementById("dtlview_Invoice Number").innerHTML;
		inno = inno.replace(/^\s+|\s+$/g, '');
		if(inno == "")
		{
			alert("'Invoice Number' must be provided first before assigning installation team.");
			return false;
		}
	}
	/*
